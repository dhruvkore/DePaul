...
public class Rectangle implements IShape {
    ...
    public Rectangle(ApplicationState applicationState){
    ...
    public void drawShape(PaintCanvas pc) {
        ...
        if(this.activeShapeShadingType.equals(ShapeShadingType.OUTLINE)){
            graphics.drawRect(topLeftX, topLeftY, x, y);
        }
        else if(this.activeShapeShadingType.equals(ShapeShadingType.FILLED_IN)) {
            graphics.fillRect(topLeftX, topLeftY, x, y);
        }
    ...
    @Override
    public IShape clone() {
        return new Rectangle(0, 0, Math.abs(this.endX - this.startX), Math.abs(this.endY - this.startY),
